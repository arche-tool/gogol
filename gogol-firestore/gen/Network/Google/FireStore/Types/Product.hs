...
-- | The response for Firestore.RunQuery.
--
-- /See:/ 'runQueryDocument' smart constructor.
data RunQueryDocument =
  RunQueryDocument'
    { _rqdReadTime :: !(Maybe DateTime')
    , _rqdSkippedResults :: !(Maybe (Textual Int32))
    , _rqdTransaction :: !(Maybe Bytes)
    , _rqdDocument :: !(Maybe Document)
    }
  deriving (Eq, Show, Data, Typeable, Generic)


-- | Creates a value of 'RunQueryDocument' with the minimum fields required to make a request.
--
-- Use one of the following lenses to modify other fields as desired:
--
-- * 'rqdReadTime'
...
runQueryDocument =
  RunQueryDocument'
    { _rqdReadTime = Nothing
    , _rqdSkippedResults = Nothing
    , _rqdTransaction = Nothing
    , _rqdDocument = Nothing
    }
...
-- query was run.
rqdReadTime :: Lens' RunQueryDocument (Maybe UTCTime)
rqdReadTime
  = lens _rqdReadTime (\ s a -> s{_rqdReadTime = a}) .
      mapping _DateTime

-- | The number of results that have been skipped due to an offset between
-- the last response and the current response.
rqdSkippedResults :: Lens' RunQueryDocument (Maybe Int32)
rqdSkippedResults
  = lens _rqdSkippedResults
      (\ s a -> s{_rqdSkippedResults = a})
      . mapping _Coerce
...
-- response.
rqdTransaction :: Lens' RunQueryDocument (Maybe ByteString)
rqdTransaction
  = lens _rqdTransaction
      (\ s a -> s{_rqdTransaction = a})
      . mapping _Bytes

-- | A query result. Not set when reporting partial progress.
rqdDocument :: Lens' RunQueryDocument (Maybe Document)
rqdDocument
  = lens _rqdDocument (\ s a -> s{_rqdDocument = a})

instance FromJSON RunQueryDocument where
        parseJSON
          = withObject "RunQueryDocument"
              (\ o ->
                 RunQueryDocument' <$>
...
instance ToJSON RunQueryDocument where
        toJSON RunQueryDocument'{..}
          = object
              (catMaybes
                 [("readTime" .=) <$> _rqdReadTime,
                  ("skippedResults" .=) <$> _rqdSkippedResults,
                  ("transaction" .=) <$> _rqdTransaction,
                  ("document" .=) <$> _rqdDocument])
...
-- | A structured query. Filters, order bys, limits, offsets, and start\/end
...
-- returned across multiple pages of results. The number must be strictly
